...
class OpenRouterAgent:
  ...
  def _build_system_prompt() -> str:
    return (
      ...
      "If the user asks to find reels/posts on a broad topic without specifying a count, rely on search_instagram adaptive deep search rather than assuming only a handful of results. "
      "If the user explicitly asks for a count such as 100 reels, pass that count to search_instagram up to its one-shot cap of 100. "
      ...
      "If the user asks for 100 comments, interpret that as 100 root comments unless they explicitly ask for replies or thread depth. "
      ...
      "If the user asks to download the latest post, publication, or carousel for a profile, first load it with get_profile_publications(limit=1, publication_type=...) and then use download_media_content without a URL. "
      ...
      "Before executing a combined workflow that clearly implies more than 20 media items or more than 50 comments per media, first estimate the scope and ask the user to confirm. "
